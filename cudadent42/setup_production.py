...
def setup_cuda_extension():
    ...
    # ⚠️ PLAN B: Separate CUDAExtension per dtype (simpler, avoids per-file flag issues)
    
    # FP16 TU: with BF16 suppression
    nvcc_fp16_flags = base_cuda_flags + [
        '-DCUDA_NO_BFLOAT16',
        '-D__CUDA_NO_BFLOAT16_OPERATORS__',
    ]
    ...
    ext_modules.append(CUDAExtension(
        ...
        extra_compile_args={
            'cxx': cxx_flags,
            'nvcc': nvcc_fp16_flags,
        },
        define_macros=define_macros,
        include_dirs=['python/flashmoe_science/csrc'],
    ))
    
    # Add BF16 extension if supported (NO suppression macros!)
    if has_bf16:
        nvcc_bf16_flags = base_cuda_flags  # NO suppression!
        ext_modules.append(CUDAExtension(
            name='flashmoe_science._C',  # Same name, linker will merge symbols
            sources=[
                'python/flashmoe_science/csrc/flash_attention_bf16_sm80.cu',
            ],
            extra_compile_args={
                'cxx': cxx_flags,  # Host flags still suppress (not used in .cu files)
                'nvcc': nvcc_bf16_flags,  # NO suppression for BF16!
            },
            define_macros=define_macros,
            include_dirs=['python/flashmoe_science/csrc'],
        ))
        print("📦 Added BF16 extension (SM80+, NO macro suppression)")
    
    return ext_modules, BuildExtension

ext_modules, build_ext_class = setup_cuda_extension()

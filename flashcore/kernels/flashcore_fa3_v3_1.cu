// flashcore_fa3_v3_1.cu
// FA-3 v3.1: Fixed state management (one row at a time per warp)
// Architecture: Inverted loops (K/V outer, Q inner) - VALIDATED 8.5× faster!
// Target: 620 μs with correct results, then add WMMA for <40 μs
...
flash3_fused_attention_fp16_kernel_v3_1(
    ...
    float scale)
{
  ...
  // Each warp processes its rows ONE AT A TIME through all K/V tiles
  ...
  for (int q_local = warp_id; q_local < q_tile_len; q_local += nwarps) {
    ...
    float q_reg[4];  // Supports up to D=128 (128/32=4)
    #pragma unroll
    ...
    float m_i = -INFINITY;
    float l_i = 0.0f;
    float out_acc[4];  // Output accumulator
    #pragma unroll
    for (int c = 0; c < elems_per_lane; ++c) {
      out_acc[c] = 0.0f;
    }
    ...
    for (int k_start = 0; k_start < S; k_start += N_TILE) {
      ...
      // All warps cooperatively load K/V tile
      {
      ...
      for (int j = 0; j < k_len; ++j) {
        ...
        // Compute Q·K[j] (scalar dot product for now, WMMA later)
        float partial = 0.0f;
        #pragma unroll
        for (int c = 0; c < elems_per_lane; ++c) {
          const int d_idx = c * 32 + lane_id;
        ...
        // Update output accumulator: O_new = O_old * alpha + beta * V[j]
        #pragma unroll
        for (int c = 0; c < elems_per_lane; ++c) {
          const int d_idx = c * 32 + lane_id;
      ...
      __syncthreads();  // Wait for all warps before loading next K/V tile
    }

    // Normalize and write output for this row
    #pragma unroll
    for (int c = 0; c < elems_per_lane; ++c) {
      const int d_idx = c * 32 + lane_id;
    ...
    cudaStream_t stream)
{
  ...
  flash3_fused_attention_fp16_kernel_v3_1<<<grid, block, smem_bytes, stream>>>(

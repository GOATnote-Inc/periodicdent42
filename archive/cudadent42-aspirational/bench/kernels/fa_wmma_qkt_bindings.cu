/*
 * PyTorch bindings for WMMA Q@K^T kernel (Phase C.1)
 */
...
torch::Tensor flash_attention_wmma_qkt_forward(
    torch::Tensor Q,
    torch::Tensor K,
    torch::Tensor V,
    float scale
    ...
    // Launch kernel
    launch_flash_attention_wmma_qkt(
    ...
    TORCH_CHECK(err == cudaSuccess, 
                "WMMA kernel launch failed: ", cudaGetErrorString(err));
    
    return O;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &flash_attention_wmma_qkt_forward, 
          "FlashAttention with WMMA Q@K^T (Phase C.1)",
          py::arg("Q"),
          py::arg("K"),
          py::arg("V"),
          py::arg("scale"));

#!/usr/bin/env python3
"""
Test FA-3 v6_opt_vec kernel (WMMA for Q·K^T (Tensor Cores!))
...
print("=" * 70)
print("FA-3 v6_opt_vec Kernel Test (v6 Optimized (Vectorized)!)")
print("=" * 70)
print()
...
print("[1/4] Building...")
try:
    import build_fa3_v6_opt_vec
    import flashcore_fa3_v6_opt_vec
    print("✅ Build OK")
...
try:
    with torch.no_grad():
        ...
        out = flashcore_fa3_v6_opt_vec.forward(Q, K, V, is_causal)
...
try:
    # Warmup
    for _ in range(100):
        flashcore_fa3_v6_opt_vec.forward(Q, K, V, is_causal)
    ...
    fa3_p50, fa3_p90 = benchmark(lambda: flashcore_fa3_v6_opt_vec.forward(Q, K, V, is_causal))
    sdpa_p50, sdpa_p90 = benchmark(lambda: torch.nn.functional.scaled_dot_product_attention(Q, K, V, is_causal=is_causal))
    
    print(f"  FA-3 v6_opt_vec:       p50={fa3_p50:.2f} μs, p90={fa3_p90:.2f} μs")

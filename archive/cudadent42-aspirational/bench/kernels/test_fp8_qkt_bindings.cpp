...
torch::Tensor test_qkt_forward(
    torch::Tensor Q_fp8,    // [M, D] uint8
    torch::Tensor K_fp8,    // [N, D] uint8
    float Q_scale,
    float K_scale
) {
    ...
    TORCH_CHECK(K_fp8.size(1) == D, "K must have same D as Q");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &test_qkt_forward, "Test Q@K^T with FP8");
}

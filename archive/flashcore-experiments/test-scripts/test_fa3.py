#!/usr/bin/env python3
"""
Test FlashAttention-3 style kernel
Goal: <40 μs (beat PyTorch SDPA's 44 μs)
"""
...
print("=" * 70)
print("FlashCore: FA-3 Kernel Test")
print("=" * 70)
print()

# Build kernel
print("[1/4] Building FA-3 kernel...")
try:
    import build_fa3
...
try:
    O_fa3 = flashcore_fa3.forward(Q, K, V)
...
if not correctness_pass:
    ...
    print(f"🎉 SUCCESS! FA-3 kernel beats target!")
    print(f"   {p50:.2f} μs < {target_us} μs ({margin:.1f}% margin)")
    print()
    if p50 < pytorch_us:
        pytorch_margin = ((pytorch_us - p50) / pytorch_us) * 100
        print(f"✅ ALSO beats PyTorch SDPA by {pytorch_margin:.1f}%")
        print(f"   {p50:.2f} μs < {pytorch_us:.2f} μs")
    print()
    print("🚀 MISSION ACCOMPLISHED!")
    print("   - Custom FA-3 kernel faster than PyTorch")
    print("   - Perfect correctness")
    print("   - Standing on giants' shoulders and going further!")
elif p50 < pytorch_us:
    margin = ((pytorch_us - p50) / pytorch_us) * 100
    print(f"✅ BEATS PyTorch SDPA by {margin:.1f}%!")
    print(f"   {p50:.2f} μs < {pytorch_us:.2f} μs")
    print(f"   Close to target (gap: {p50 - target_us:.2f} μs)")
    print()
    print("Next: Profile with NCU and apply micro-optimizations")
else:
    ...
    print("  - Add WMMA for Tensor Cores")
    print("  - Use cp.async for overlapped loads")
    print("  - Tune tile sizes (M_TILE, N_TILE)")

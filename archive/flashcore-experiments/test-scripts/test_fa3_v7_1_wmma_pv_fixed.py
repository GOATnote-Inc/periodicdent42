#!/usr/bin/env python3
"""
Test FA-3 v7_1_wmma_pv_fixed kernel (WMMA for Q·K^T (Tensor Cores!))
...
print("=" * 70)
print("FA-3 v7_1_wmma_pv_fixed Kernel Test (Phase 2.1: WMMA FULL FIXED (QK^T + PV)!)")
print("=" * 70)
print()
...
print("[1/4] Building...")
try:
    import build_fa3_v7_1_wmma_pv_fixed
    import flashcore_fa3_v7_1_wmma_pv_fixed
    print("✅ Build OK")
...
try:
    with torch.no_grad():
        ...
        out = flashcore_fa3_v7_1_wmma_pv_fixed.forward(Q, K, V, is_causal)
...
try:
    # Warmup
    for _ in range(100):
        flashcore_fa3_v7_1_wmma_pv_fixed.forward(Q, K, V, is_causal)
    ...
    fa3_p50, fa3_p90 = benchmark(lambda: flashcore_fa3_v7_1_wmma_pv_fixed.forward(Q, K, V, is_causal))
    sdpa_p50, sdpa_p90 = benchmark(lambda: torch.nn.functional.scaled_dot_product_attention(Q, K, V, is_causal=is_causal))
    
    print(f"  FA-3 v7_1_wmma_pv_fixed:       p50={fa3_p50:.2f} μs, p90={fa3_p90:.2f} μs")
...
print(f"Performance:")
print(f"  FA-3 v7_1_wmma_pv_fixed:       {fa3_p50:.2f} μs")
print(f"  PyTorch SDPA:  {sdpa_p50:.2f} μs (baseline)")
print()

if correct:
    print("🎉 SUCCESS - Phase 2.1: WMMA FULL FIXED (QK^T + PV) (WMMA Tensor Cores!)!")
    print()
    print("Performance progression:")
    print("  v2 (wrong arch):      5259 μs  (per-row K/V reload ❌)")
    print("  v3.1 (Q outer):       2891 μs  (still wrong order ❌)")
    print(f"  v7_1_wmma_pv_fixed (K/V outer):        {fa3_p50:.2f} μs  (CORRECT! ✅)")

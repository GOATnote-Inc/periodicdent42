...
from services.llm.client import LLMClient, Message
...
class ChatPipeline:
    def __init__(self, llm_client: LLMClient, index: CorpusIndex, telemetry_store: TelemetryStore):
        self.llm_client = llm_client
        self.index = index
        self.telemetry_store = telemetry_store

    @classmethod
    def default(cls, telemetry_store: TelemetryStore) -> "ChatPipeline":
        index = ingest()
        llm_client = LLMClient.fake()
        return cls(llm_client=llm_client, index=index, telemetry_store=telemetry_store)

    def run(self, request: ChatRequest, router_decision: RouterDecision | None) -> ChatResponse:
...
__all__ = ["ChatPipeline", "ChatRequest", "ChatResponse", "Citation", "VectorStats"]

#!/usr/bin/env python3
"""
FP8 SDPA Vectorized Kernel - Cycle 2a Optimization

Vectorization optimizations:
1. int4 (16-byte) loads/stores for Q/K/V
2. Q dequant once per row/lane (reuse across K loop)
3. cp.async for K/V tiles (Ampere/Ada)
4. Explicit __align__(16) for SMEM arrays
5. Numerical robustness (epsilon on l_final)

Target: 400-500 μs (3-4× from 1596 μs baseline)
"""
...
def build_module():
    print("\n" + "="*80)
    print("BUILDING FP8 SDPA VECTORIZED KERNEL")
    print("="*80)
    print("\nTarget: L4 (sm_89, Ada)")
    print("Optimizations:")
    print("  ✅ int4 (16-byte) vectorized loads/stores")
    print("  ✅ Q dequant once per row/lane")
    print("  ✅ cp.async for K/V (Ampere/Ada)")
    print("  ✅ Explicit __align__(16) on SMEM")
    print("  ✅ Numerical robustness (epsilon guard)")
    print("\nCompiling sdpa_fp8_vectorized.cu...")
    ...
    module = load(
        name="sdpa_fp8_vectorized",
        sources=[
            str(kernel_dir / "sdpa_fp8_vectorized.cu"),
            str(kernel_dir / "sdpa_fp8_vectorized_bindings.cpp"),
        ],
        extra_cuda_cflags=[
            "-O3",
            "-use_fast_math",
...
def main():
    ...
    print("="*80)
    print("BENCHMARK: FP8 SDPA VECTORIZED (Cycle 2a)")
    print("="*80)
    ...
    # Results
    print("\n" + "="*80)
    print("RESULTS: FP8 SDPA VECTORIZED (Cycle 2a)")
    print("="*80)
    ...
    if speedup_vs_baseline >= 1.0:
        ...
        print(f"  ⚠️ {1.0/speedup_vs_baseline:.2f}× slower")
    ...
    if latency_us < champion_us:
        ...
        print(f"  ⚠️ {latency_us/champion_us:.2f}× slower (still room for optimization)")
    
    # vs Target
    target_us = 500.0
    print(f"\nCycle 2a Target: {target_us:.1f} μs (vectorization)")
    ...
    if correct and speedup_vs_baseline >= 2.0:
        print("✅ CYCLE 2a SUCCESS - Vectorization working!")
        print("="*80)
        print("\nNext steps:")
        print("  ✅ Vectorization complete")
        if latency_us <= 500:
            print("  ✅ Target met - Ready for Cycle 3 (cp.async double-buffering)")
        else:
            print("  → Consider Cycle 3 anyway (cp.async may help more)")
    else:
        ...
        if speedup_vs_baseline < 2.0:
            print(f"\nVectorization speedup below expected: {speedup_vs_baseline:.2f}× < 2.0×")
